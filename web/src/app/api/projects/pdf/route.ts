...
function drawList(doc: PDFKit.PDFDocument, html: string, ordered: boolean) {
  ...
  for (const item of items) {
    ...
    const y = doc.y;
    doc.font("Helvetica").fontSize(11).fillColor(DEFAULT_TEXT).text(prefix, left, y, { lineBreak: false, continued: false });
    drawStyledInline(doc, raw, 11, { x: left + indent, y, width: textWidth, color: DEFAULT_TEXT });
