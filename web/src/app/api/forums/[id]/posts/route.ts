...
export async function GET(_req: NextRequest, ctx: Ctx) {
  ...
  const posts = await db
    .select({
      ...
      mediaPath: forumPosts.mediaPath,
    ...
    .where(eq(forumPosts.forumId, id))
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  const { id } = await ctx.params;
  const { forum, allowed: canPost } = await canAccessForum(id, session.id);
  if (!forum) return NextResponse.json({ error: "Not found" }, { status: 404 });
  ...
  const kindOptions: PostKind[] = ["text", "voice", "system", "image", "file"];
  const kind: PostKind = kindOptions.includes(body.kind as PostKind)
    ? (body.kind as PostKind)
    : "text";

  if (kind === "text" && !text && !body.mediaPath) {
  ...
  const now = Math.floor(Date.now() / 1000);
  const displayBody =
    text || (kind === "voice" ? "Voice note" : body.mediaPath ? "Attachment" : "");
  ...
  return NextResponse.json(
    {
      ...
      authorColor: session.avatarColor,
    },
    { status: 201 }
  );
}
